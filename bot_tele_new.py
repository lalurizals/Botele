...
API_TOKEN = '1854541892:AAH_T-i5rjR1OwtcoRs6WIrS_TknRxB0AB0'
...
def handle_photo_message(message):
   ...
   cursor.execute("insert into public.laporan (chatid, username, firstname, lastname, gambar, kode, tanggal) values (%s, %s, %s, %s, %s, %s, %s)", (chatid, username, firstname, lastname, filefix2, kodefix2, tanggal))
   mydb.commit()
   
   image = Image.open(io.BytesIO(filefix))
   #image.show()

   bot.send_message(message.chat.id, "Terimakasih atas laporan anda \nKode laporan: " + kodefix2)
